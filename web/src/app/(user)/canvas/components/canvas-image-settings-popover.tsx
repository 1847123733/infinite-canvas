...
import { isDoubaoImageModel, normalizeDoubaoImageResolution, resolveDoubaoImageAspect } from "@/lib/doubao-image";
...
export function CanvasImageSettingsPopover({ config, onConfigChange, onOpenChange, buttonClassName, placement = "topLeft" }: CanvasImageSettingsPopoverProps) {
    ...
    const summary = isDoubao ? `${normalizeDoubaoImageResolution(config.imageResolution).toUpperCase()} · ${resolveDoubaoImageAspect(activeSize) === "auto" ? "智能" : resolveDoubaoImageAspect(activeSize)}` : `${imageQualityLabel(quality)} · ${imageSizeLabel(activeSize)}`;
