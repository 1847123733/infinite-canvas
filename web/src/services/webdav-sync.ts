...
import { appApiUrl, isDesktopRuntime } from "@/services/api/url";
...
async function webdavFetch(config: WebdavSyncConfig, path: string, init: RequestInit) {
    ...
    try {
        ...
        if (config.proxyMode === "nextjs") {
            const proxyUrl = isDesktopRuntime() ? appApiUrl("/api/webdav-proxy") : "/api/webdav-proxy";
            return await fetch(proxyUrl, { method: "POST", headers: proxyHeaders(url, init.method || "GET", headers), body: proxyBody(init), signal: controller.signal });
        }
