...
function seedanceApiUrl(config: AiConfig, taskId?: string) {
    if (config.channelMode === "remote") return appApiUrl(taskId ? `/api/v1/videos/${encodeURIComponent(taskId)}` : "/api/v1/videos");
...
async function uploadReferenceMedia(file: File) {
    ...
    const response = await axios.post<ApiEnvelope<ReferenceMediaUploadResponse>>(appApiUrl("/api/v1/media/references"), body, { headers: { Authorization: `Bearer ${token}` } });
