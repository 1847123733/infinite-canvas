...
import { CanvasStorageModal } from "@/app/(user)/canvas/components/canvas-storage-modal";
            ...
            >
                <div className="pt-1">
                    <Form layout="vertical" requiredMark={false}>
                        ...
                        <Form.Item label="画布存储" extra="查看本地图片、音视频占用，并安全清理所有画布和素材均未引用的文件。" className="mb-0">
                            <div className="flex items-center justify-between gap-4 rounded-lg border border-stone-200 px-4 py-3 dark:border-stone-800">
                                <div className="flex min-w-0 items-center gap-3">
                                    <HardDrive className="size-5 shrink-0 text-stone-500" />
                                    <div>
                                        <div className="font-medium text-stone-900 dark:text-stone-100">本地画布缓存</div>
                                        <div className="mt-0.5 text-xs text-stone-400">清理无引用文件不会影响现有画布和“我的素材”。</div>
                                    </div>
                                </div>
                                <Button onClick={() => setStorageOpen(true)}>查看与清理</Button>
                            </div>
                        </Form.Item>
                    </Form>
                </div>
            </Modal>
            <CanvasStorageModal open={storageOpen} onClose={() => setStorageOpen(false)} />
        </>
    );
}
